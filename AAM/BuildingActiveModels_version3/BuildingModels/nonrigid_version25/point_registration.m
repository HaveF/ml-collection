...
% Correct for scaling differences
mm=mean(Xmoving,1); ms=mean(Xstatic,1);
Rm=mean(sqrt(sum(bsxfun(@plus,Xmoving,-mm),2).^2));
Rs=mean(sqrt(sum(bsxfun(@plus,Xstatic,-ms),2).^2));
O_add=O_add+O_ref*Rs/Rm;

% Warp the points
Xreg=bspline_trans_points_double((O_ref+O_add),Spacing,Xmoving);

% Correct for translation
mm=mean(Xreg,1); ms=mean(Xstatic,1);
cm=ms-mm;
switch(size(Xstatic,2))
    case 2
        O_add(:,:,1)=O_add(:,:,1)+cm(1);
        O_add(:,:,2)=O_add(:,:,2)+cm(2);
    case 3
        O_add(:,:,:,1)=O_add(:,:,:,1)+cm(1);
        O_add(:,:,:,2)=O_add(:,:,:,2)+cm(2);
        O_add(:,:,:,3)=O_add(:,:,:,3)+cm(3);
end

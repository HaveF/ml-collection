...
voidthread transformvolume_color(double **Args) {
    ...
    index_rgb[0]=0; index_rgb[1]=ImageSize[0]*ImageSize[1]; index_rgb[2]=2*index_rgb[1];
   ...
   double *Bu, *Bv, *Iout, *Tx, *Ty, *dxa, *dya, *ThreadID, *Ox, *Oy, *Iin, *moded;
    ...
    for (y=ThreadOffset; y<Isize[1]; y=y+Nthreads)
    {
        ...
        for (x=0; x<Isize[0]; x++)
        {
            ...
            /*  Store transformation field */
            Tx[indexI]=Tlocalx; Ty[indexI]=Tlocaly;
        }
    }
                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    /* dx and dy are the spacing of the b-spline knots */
    double *Ox,*Oy, *Iin, *dxa, *dya, *Iout,*Tx,*Ty,*moded, *ImageSizeT;
  ...
  Isize[1] = (int)Isize_d[1]; 
  Isize[2] = (int)Isize_d[2]; 
  if(nlhs>1) { plhs[1] = mxCreateDoubleMatrix(Isize[0],Isize[1], mxREAL); }
  if(nlhs>2) { plhs[2] = mxCreateDoubleMatrix(Isize[0],Isize[1], mxREAL); }
  ...
  if(nlhs<2&&moded[0]==4)
  {
      Tx = (double*)malloc(Isize[0]*Isize[1]*sizeof(double));
      Ty = (double*)malloc(Isize[0]*Isize[1]*sizeof(double));
  }
  ...
  if(moded[0]==4)
  {
      interpolate_forward_2d_double(Iin, Tx, Ty, Isize, ImageSize, Iout);

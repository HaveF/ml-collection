...
if(nargout>1)
    ...
    for zi=0:3,
        for zj=0:3,
            ...
            C_gradpx=deterror(C_gradpx);
            C_gradpy=deterror(C_gradpy);
...
function Ierr=deterror(Idet)
Ierr=zeros(size(Idet));
check=Idet~=0;
Ierr(check)=abs(log(Idet(check)))./Idet(check); 
check=Idet<0.01;
Ierr(check)=5.6002032713-99.5033085317*Idet(check);

...
voidthread transformvolume_gray(float **Args) {
...
voidthread transformvolume_forward(float **Args) {
    float *Bu, *Bv, *Bw, *Iout, *Tx, *Ty, *Tz, *moded;
    float *dxa, *dya, *dza, *ThreadID, *Ox, *Oy, *Oz, *Iin;
    float *Isize_d;
    float *Osize_d;
                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    float *Ox,*Oy,*Oz,*Iin, *dxa, *dya,*dza,*Iout, *Tx,*Ty,*Tz,*moded, *ImageSizeT;
  ...
  if(nrhs==9)
  {
      ImageSizeT=(float *)mxGetData(prhs[8]);
  }
  ...
  if(nlhs<2&&moded[0]==4)
  {
      Tx = (float*)malloc(Isize[0]*Isize[1]*Isize[2]*sizeof(float));
      Ty = (float*)malloc(Isize[0]*Isize[1]*Isize[2]*sizeof(float));
      Tz = (float*)malloc(Isize[0]*Isize[1]*Isize[2]*sizeof(float));
  }
...
if(nrhs==9)
  ...
  for (i=0; i<Nthreads; i++)
  {
    ...
    ThreadArgs1 = (float **)malloc( 21* sizeof( float * ) );  
	ThreadArgs1[0]=Bu;
	ThreadArgs1[1]=Bv;
	ThreadArgs1[2]=Bw;
...
for (i=0; i<Nthreads; i++) { WaitForThreadFinish(ThreadList[i]); }
  ...
  if(moded[0]==4)
  {
      interpolate_forward_3d_single(Iin, Tx, Ty, Tz, Isize, Isize, Iout);
        ...
        for (z=0; z<Isize[2]; z++)
        {
            for (y=0; y<Isize[1]; y++)
            {
                for (x=0; x<Isize[0]; x++)
                {
                  ...
                  Tx[index]=Tx[index]-(float)x;
                  Ty[index]=Ty[index]-(float)y;
                  Tz[index]=Tz[index]-(float)z;
                }
            }
        }
      }

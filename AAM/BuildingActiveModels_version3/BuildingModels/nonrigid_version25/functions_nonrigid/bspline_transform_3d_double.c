 ...
 * function [Vout,Tx,Ty,Tz]t=bspline_transform_3d_double(Ox,Oy,Oz,Vin,dx,dy,dz,mode)
 * 
...
voidthread transformvolume_gray(double **Args) {
	...
	u_index_array= (int*)malloc(ImageSize[0]* sizeof(int));
	i_array= (int*)malloc(ImageSize[0]* sizeof(int));
	v_index_array= (int*)malloc(ImageSize[1]* sizeof(int));
	j_array= (int*)malloc(ImageSize[1]* sizeof(int));
	w_index_array= (int*)malloc(ImageSize[2]* sizeof(int));
	k_array= (int*)malloc(ImageSize[2]* sizeof(int));
	...
	for (z=ThreadOffset; z<ImageSize[2]; z++) {
     ...
     for (z=ThreadOffset; z<ImageSize[2]; z=z+Nthreads)
     {
	    ...
	    for (y=0; y<ImageSize[1]; y++)
        {
            ...
            for (x=0; x<ImageSize[0]; x++)
            {
                ...
                indexI=x+y*ImageSize[0]+z*ImageSize[0]*ImageSize[1];
...
voidthread transformvolume_forward(double **Args) {
    double *Bu, *Bv, *Bw, *Iout, *Tx, *Ty, *Tz, *moded;
    double *dxa, *dya, *dza, *ThreadID, *Ox, *Oy, *Oz, *Iin;
    double *Isize_d;
    double *Osize_d;
                  ...
                  int nrhs, const mxArray *prhs[] )
{
    ...
    double *Ox,*Oy,*Oz,*Iin, *dxa, *dya,*dza,*Iout, *Tx,*Ty,*Tz,*moded, *ImageSizeT;
  ...
  if(nrhs==9)
  {
      ImageSizeT=(double *)mxGetData(prhs[8]);
  }
  ...
  if(nlhs<2&&moded[0]==4)
  {
      Tx = (double*)malloc(Isize[0]*Isize[1]*Isize[2]*sizeof(double));
      Ty = (double*)malloc(Isize[0]*Isize[1]*Isize[2]*sizeof(double));
      Tz = (double*)malloc(Isize[0]*Isize[1]*Isize[2]*sizeof(double));
  }
  ...
  Isize_d[0]=(double)Isize[0];  Isize_d[1]=(double)Isize[1]; Isize_d[2]=(double)Isize[2];
  ...
  /* Create image matrix for the return arguments with the size of ImageSize  */ 
  ...
  for (i=0; i<Nthreads; i++)
  {
    ...
    ThreadArgs1 = (double **)malloc( 21* sizeof( double * ) );  
	ThreadArgs1[0]=Bu;
	ThreadArgs1[1]=Bv;
	ThreadArgs1[2]=Bw;
    ...
    ThreadArgs1[20]=ImageSize_d;
       
    ThreadArgs[i]=ThreadArgs1;

    if(moded[0]==4)
    {
        StartThread(ThreadList[i], &transformvolume_forward, ThreadArgs[i])
    }
    else
    {
...
for (i=0; i<Nthreads; i++) { WaitForThreadFinish(ThreadList[i]); }
  ...
  if(moded[0]==4)
  {
      interpolate_forward_3d_double(Iin, Tx, Ty, Tz, Isize, ImageSize, Iout);

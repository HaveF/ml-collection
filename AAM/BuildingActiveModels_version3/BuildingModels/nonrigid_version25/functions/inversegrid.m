...
if(ndims(O_trans)==3)
    ...
    O_ref = make_init_grid(Spacing/8,[sizeI(1) sizeI(2) sizeI(3)]);
    x=O_ref(:,:,:,1); 
    y=O_ref(:,:,:,2);
    z=O_ref(:,:,:,3);
    x=x(:);
    y=y(:);
    z=z(:);
    check=(x<1)|(x>sizeI(1))|(y<1)|(y>sizeI(2))|(z<1)|(z>sizeI(3));
    x(check)=[]; y(check)=[]; z(check)=[];
    X2(:,1) = x(:); X2(:,2) = y(:); X2(:,3) = z(:);
    X1 = bspline_trans_points_double(O_trans,Spacing,X2);
    Options.MaxRef=MaxRef;
    [O_trans2,Spacing2]=point_registration([sizeI(1) sizeI(2) sizeI(3)],X1,X2,Options);    
end

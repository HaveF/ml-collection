...
voidthread jacobian_error(double **Args) {
    ...
    for (y=ThreadOffset; y<Isize[1]; y=y+Nthreads) {
        ...
        for (x=0; x<Isize[0]; x++) {
			...
			LocalTransformation2D(i,j,Bu, Bv, Bdu,Bdv, u_index, v_index, Ox, Oy, Osize, Tlocald);

            /* Set the current pixel value */
            Idet=Tlocald[0]*Tlocald[3]-Tlocald[2]*Tlocald[1];
...
voidthread jacobian_errorgradient(double **Args) {
    ...
    for (y=ThreadOffset; y<Isize[1]; y=y+Nthreads) {
        ...
        for (x=0; x<Isize[0]; x++) {
            ...
            /*  This part calculates the coordinates of the pixel */
            /*  which will be transformed to the current x,y pixel. */
            LocalTransformation2DGradient(i,j,Bu, Bv, Bdu,Bdv, u_index, v_index, Ox, Oy, Osize, nCPinvolved, Tlocald,TlocalGradxx, TlocalGradxy,TlocalGradyx, TlocalGradyy,CPinvolved);
		    ...
		    for(k1=0; k1<16; k1++)
			{
				TlocalGradxx[k1]+=Tlocald[0]; TlocalGradxy[k1]+=Tlocald[1];
				TlocalGradyx[k1]+=Tlocald[2]; TlocalGradyy[k1]+=Tlocald[3];
			}
            ...
            for(k1=0; k1<nCPinvolved[0]; k1++) 
			{
                ...
                Idet=TlocalGradxx[k]*Tlocald[3]-Tlocald[2]*TlocalGradxy[k];
            	err_gradientx[indexO]+=deterror(Idet)-Ierr;
                
                Idet=Tlocald[0]*TlocalGradyy[k]-TlocalGradyx[k]*Tlocald[1];
